...
# grabbed from https://github.com/tcatm/halfnode
...
class msg_version(object):
    ...
    def deserialize(self, f):
        self.nVersion = struct.unpack("<i", f.read(4))[0]
        debuglog('btcnet', 'nVersion = ', self.nVersion)
...
class BitcoinNode(threading.Thread):
    ...
    def run(self):
        ...
        while not self.e_stop.isSet():
            ...
            debuglog('btcnet', "connecting %s" % self.dstaddr)
        ...
        while True:
            ...
            if self.recvbuf[:4] != "\xf9\xbe\xb4\xd9":
                ...
                if skip == -1:
                    ...
                    debuglog('btcnet', "Lost stream sync. Discarding first %i bytes" % skip)
                    debuglog('btcnet', "Buffer contents: %s" % repr(self.recvbuf))
            ...
            if command in self.messagemap:
                ...
                debuglog('btcnet', "UNKNOWN COMMAND", command, repr(msg))
    def send_message(self, message):
        if self.state != "connected":
            return
        debuglog('btcnet', "send %s" % repr(message))
    ...
    def got_message(self, message):
        ...
        debuglog('btcnet', "got", message.command)
